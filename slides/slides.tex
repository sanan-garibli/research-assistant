...
% Remaining red [TODO: ...] markers: team name, members B/C.
% =====================================================================
...
\begin{frame}{The problem in one sentence}
  ...
  \textbf{Stack:} Anthropic \texttt{claude-opus-5} for synthesis, Tavily for web search, keyless Wikipedia + arXiv.
\end{frame}
...
\begin{frame}{Concurrency: the benchmark}
  ...
  \begin{tabular}{lcc}
    ...
    \midrule
    5 questions, live APIs, wall-clock (s) & 46.8 & 11.7 \\
    Speedup & 1.0$\times$ & $4.0\times$ \\
  ...
  \textbf{Why $4.0\times$, not $5\times$:} synthesis is 91\,\% of sequential time; concurrently all five LLM calls overlap, so wall-clock $\approx$ the slowest question ($\sim$1--2\,s fetch + $\sim$10\,s LLM). \texttt{Semaphore(5)} was not the limiter: all 15 fetches finished in $\sim$2\,s.

  \vspace{0.2em}
  {\footnotesize\textit{Run 12 Sep 2026: Python 3.14.3, \texttt{claude-opus-5}, Tavily. An earlier run the same day gave 71.6\,s vs 11.8\,s ($6.1\times$) because arXiv hit its 10\,s timeout in the sequential phase.}}
\end{frame}
...
\begin{frame}{Testing}
  ...
  \begin{itemize}\setlength\itemsep{0.3em}
    \item Total coverage on \texttt{researcher/}: \textbf{95\,\%} (432 stmts, 22 missed)
    \item Provided \texttt{ai/} smoke tests: \textcolor{ok}{\textbf{passing}}, 16/16 unmodified
    \item Test count: \textbf{67 own + 16 smoke = 83}, 2.1\,s
...
\begin{frame}{Limitations \& what we'd do next}
  ...
  \begin{itemize}\setlength\itemsep{0.3em}
    ...
    \item Synthesised answers are not cached: a demo pass with 15/15 source-cache hits was only 0.8\,s faster (48.3\,s $\to$ 47.5\,s).
    \item Wikipedia matches titles by prefix; our keyword fallback is an English-stopword heuristic tuned on the 5 sample questions.
  \end{itemize}
  ...
  \begin{enumerate}\setlength\itemsep{0.3em}
    ...
    \item Repeat the live benchmark for an error bar and run a 20-question load test; add a \texttt{--json} CLI mode and OpenTelemetry spans.
  \end{enumerate}
\end{frame}
