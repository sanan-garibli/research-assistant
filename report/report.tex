...
% extra members, Docker image size).
% =====================================================================
...
% microtype improves line breaking, but it is not present in every MiKTeX
% install; compile without it rather than failing outright.
...
\usepackage{listings}
% Base tcolorbox is enough for the two boxes below; the [most] option drags in
% tikzfill, which is not in every MiKTeX install.
\usepackage{tcolorbox}
% titlesec and lastpage are cosmetic; guard them so a minimal TeX install can
% still build this report (Overleaf and a full MiKTeX have both).
\IfFileExists{titlesec.sty}{\usepackage{titlesec}}{}
\usepackage{fancyhdr}
\IfFileExists{lastpage.sty}
  {\usepackage{lastpage}\newcommand{\pagemark}{Page \thepage\ of \pageref{LastPage}}}
  {\newcommand{\pagemark}{Page \thepage}}
\usepackage{tikz}
\usetikzlibrary{shapes.geometric,arrows.meta,positioning,fit,backgrounds}
\usepackage[hidelinks]{hyperref}

% Nothing may stick out into the right margin: a word that does not fit moves
% to the next line instead. We accept loose inter-word spacing as the price
% (\sloppy + a generous \emergencystretch), and silence the resulting
% underfull-line chatter.
\emergencystretch=3em
\hbadness=10000
\hfuzz=0pt
\sloppy

% Code is never hyphenated: a break inside "orchestrator.py" reads as part of
% the identifier. Long identifiers are instead split at an existing "/" by
% writing them as two \texttt groups where a line would otherwise overflow.
...
% \makeatletter must be in effect *before* the conditional is tokenised, or the
% internal \@startsection / \@plus / \z@ in the fallback branch are read as
...
\IfFileExists{titlesec.sty}{%
  ...
  % Without titlesec, keep the accent-coloured headings via plain LaTeX so the
  % report looks the same on a minimal TeX install.
...
\begin{center}
% Every node has a fixed text width so its label wraps inside the box, and the
% whole picture is scaled to the text block so it cannot reach into the margins.
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  node distance=7mm and 11mm,
  box/.style={draw,rounded corners=2pt,text width=3.0cm,align=center,
              minimum height=1.0cm,font=\scriptsize,inner sep=3pt},
  ...
  lbl/.style={font=\tiny\itshape,text=black!65,align=center,
              fill=white,inner sep=1pt}
]
  % --- row 1: entry points, then the composition root ------------------
  % Every label is broken by hand at a "/" so nothing hyphenates or overflows.
  \node[entry] (cli) {\texttt{cli.py}\\\texttt{ask} / \texttt{demo}};
  \node[entry,right=of cli] (bench) {\texttt{scripts/}\\\texttt{bench.py}};
  \node[boot,right=of bench] (boot) {\texttt{bootstrap.py}\\composition root};
  \node[boot,right=of boot] (cfg) {\texttt{config.py}\\\texttt{Settings}};

  % --- row 2: facade, orchestrator, cache ------------------------------
  \node[core,below=of cli] (fac) {\texttt{core/}\\\texttt{researcher.py}\\facade};
  \node[core,right=of fac] (orch) {\texttt{concurrency/}\\\texttt{orchestrator.py}\\gather, timeout,\\\texttt{Semaphore(5)}};
  \node[svc,right=of orch] (cache) {\texttt{services/}\\\texttt{cache.py}\\TTL cache};

  % --- row 3: AI service and the cache backend -------------------------
  \node[svc,below=of orch] (ais) {\texttt{services/}\\\texttt{ai\_service.py}\\retries, timing};
  \node[store,below=of cache] (cs) {\texttt{storage/}\\\texttt{cache\_store.py}\\backend ABC};

  % --- row 4: provided package and the outside world -------------------
  \node[aibox,below=of ais] (ai) {provided \texttt{ai/}\\(unmodified)};
  \node[ext,right=of ai] (net) {Wikipedia, arXiv,\\Tavily, LLM};

  % --- runtime calls (solid) -------------------------------------------
  % Labels sit on the arrow itself (midway, white-filled) so they cannot
  % collide with a box border.
  % pos keeps these two labels off the dashed enclosure line they would
  % otherwise sit on.
  \draw[arrow] (cli) -- node[lbl,pos=0.25] {\texttt{ask()}} (fac);
  \draw[arrow] (bench.south) -- (fac.north east);
  % The two horizontal arrows inside a row are too short to carry a label
  % without touching a box; the paragraph below names those calls instead.
  \draw[arrow] (fac) -- (orch);
  \draw[arrow] (fac.south) |- node[lbl,pos=0.72] {\texttt{synthesize()}} (ais.west);
  \draw[arrow] (orch) -- (cache);
  \draw[arrow] (orch) -- node[lbl,midway] {\texttt{fetch\_*}} (ais);
  \draw[arrow] (cache) -- (cs);
  \draw[arrow] (ais) -- node[lbl,pos=0.78] {AI boundary} (ai);
  \draw[arrow,dashed] (ai) -- (net);

  % --- construction (dotted) -------------------------------------------
  \draw[arrow] (boot) -- node[lbl,midway] {reads} (cfg);
  \draw[wire] (boot) -- (cache);

  \begin{scope}[on background layer]
    \node[fit=(fac)(orch)(cache)(ais)(cs),draw=accent!40,dashed,
          rounded corners=4pt,inner sep=5pt] (se) {};
  \end{scope}
\end{tikzpicture}}
\end{center}

\noindent{\small\textit{Dashed enclosure: \texttt{researcher/}, our SE layer.}}

\textbf{Interpreting the diagram.} Solid arrows are runtime calls; dotted arrows are construction. Both entry points call \texttt{bootstrap.build\_researcher()}, which reads \texttt{config.py} and wires backend $\to$ cache $\to$ AI service $\to$ orchestrator $\to$ facade; neither entry point constructs a domain object itself. Exactly one arrow crosses the AI-module boundary: \texttt{ai\_service.py} $\to$ \texttt{ai/}. The storage boundary is crossed only by \texttt{services/cache.py} $\to$ \texttt{storage/cache\_store.py}, in one direction; the orchestrator never touches a file. \texttt{models.py} (\texttt{SourceOutcome}, \texttt{ResearchSession}) is the typed payload carried along every arrow inside \texttt{researcher/}. Swapping Anthropic for OpenAI changes \emph{zero} files: it is \texttt{LLM\_PROVIDER=openai} plus a key, because the provided \texttt{ai.providers.factory} does the dispatch. Swapping the whole \texttt{ai/} package for a different one would change one file.
...
\midrule
5 sample questions, \texttt{for} loop vs.\ \texttt{gather} (live APIs, run 2) & 5 & 46.8 & 11.7 & $4.0\times$ \\
Same workload, run 1 (arXiv hit its 10\,s timeout once, sequential phase) & 5 & 71.6 & 11.8 & $6.1\times$ \\
1 question, 3 fetches of 0.2\,s each (offline unit benchmark) & 3 & 0.60 (sum) & $<0.40$ & $\geq 1.5\times$ \\
1 question, 1 fetch stalls 5\,s, timeout 0.2\,s (offline) & 2 & 5.05 & $<1.00$ & $>5\times$ \\
\bottomrule
\end{tabularx}
\end{center}

\textbf{Setup.} Machine: Windows 11 Enterprise laptop, Python 3.14.3 in the project virtualenv (the Docker image bakes Python 3.12); synthesis with \texttt{claude-opus-5}, web search via Tavily; both runs on 12 September 2026, back to back, over a network without TLS interception. Caches are bypassed structurally, not just cleared: \texttt{scripts/bench.py} calls \texttt{researcher.ask(q, use\_cache=False)} in both phases and builds a fresh \texttt{Researcher} (fresh semaphore, fresh HTTP client) per phase so no pooled connections leak from the sequential run into the concurrent one. Command line: \texttt{python scripts/bench.py --limit 5}. The two offline rows come from \texttt{tests/test\_orchestrator.py} (\texttt{test\_sources\_fetched\_in\_parallel\_not\_sequentially} and \texttt{test\_slow\_source\_times\_out\_without\_blocking\_fast\_sibling}) and run on every \texttt{pytest} invocation.

\textbf{Discussion.} Run 2 is the headline because every source answered in both phases. Run 1 is kept because it shows the robustness argument in numbers: in its sequential phase arXiv stalled until the 10\,s per-source timeout on the first question and Wikipedia answered in 2.4--3.6\,s per call (0.1--0.4\,s in the concurrent phase), which inflated the sequential total to 71.6\,s, while the concurrent phase came in at 11.8\,s, the same as run 2. Where the time goes explains the $4.0\times$. In run 2 the five synthesis calls took 6.5--10.2\,s each and account for 42.7\,s of the 46.8\,s sequential total (91\,\%). Concurrently, all 15 fetches, including the Wikipedia retries described below, finished within about 2\,s despite \texttt{Semaphore(5)}, and the five LLM calls then overlapped completely (5.7--10.0\,s each, no 429), so the wall clock is set by the \emph{slowest} question rather than by the semaphore or the thread pool. The achievable speedup is therefore $\sum_q t_q / \max_q t_q$, not $N$, and with synthesis latencies this uneven it stays below $5\times$. We had expected the semaphore and the \texttt{to\_thread} pool to be the limiters; at $N=5$ neither was, and the remaining lever is caching synthesised answers. The live run also exposed a data-quality bug that no offline test could: the provided \texttt{fetch\_wikipedia} uses Wikipedia's prefix title search, so all five full-sentence questions returned \emph{zero} Wikipedia sources. \texttt{AIService} now retries with shorter keyword phrases (\S\ref{sec:robustness}); in both runs Wikipedia matched every question on the first or second term (e.g.\ \texttt{"2008 financial crisis"}, \texttt{"fusion energy"}), and answers are now built from 7--9 sources instead of at most 6. The TLS-intercepting proxy that blocked the first attempt at this benchmark (\S\ref{sec:robustness}) was not in the path this time.
...
In the real TLS incident, however, the log at \texttt{INFO} was \textbf{empty}. We expected to see the certificate error; we saw nothing, because (a) \texttt{AIService} logs a fetch only after it succeeds, so three failed attempts produce zero lines, and (b) \texttt{gather\_sources} stores \texttt{str(result)} in \texttt{SourceOutcome.error} but never logs it, and \texttt{NoSourcesAvailableError} carries only the origin names, not the messages. The log did \emph{not} give us enough to diagnose the incident without running code again: we had to write a probe script that called the fetchers directly and unwrapped \texttt{\_\_cause\_\_} to find the revocation error. The test surfaced a real design gap, diagnosability of degraded sources, and two concrete fixes are queued: log the exception at \texttt{WARNING} in the \texttt{isinstance(result, BaseException)} branch of \texttt{gather\_sources}, and pass \texttt{before\_sleep=before\_sleep\_log(logger, WARNING)} to the tenacity policy so each retry is visible. Two earlier incidents of the same family were fixed during development and are visible in the code: Wikipedia returned 403 to the default User-Agent (fixed with an explicit agent string) and arXiv's \texttt{http://} endpoint 301-redirected to \texttt{https://}, which \texttt{httpx} surfaces as an error unless \texttt{follow\_redirects=True}. A third surfaced while collecting the numbers in \S\ref{sec:concurrency}: redirecting the demo's stdout to a file on Windows encodes it with the legacy cp1252 code page, so the first answer containing CO\textsubscript{2} written with a subscript killed the run with a \texttt{UnicodeEncodeError} \emph{after} the LLM call had been paid for. \texttt{cli.main} now reconfigures stdout and stderr to UTF-8 before anything is printed, pinned by \texttt{test\_ask\_prints\_non\_latin1\_characters\_to\_a\_redirected\_stream}.
...
\textbf{Hit rate.} Structurally, running \texttt{python -m researcher demo} twice yields 0/15 hits on the first pass and 15/15 on the second (every \texttt{(source, question)} pair is repeated within the TTL), and a single repeated \texttt{ask} yields 3/3. Observed on 12 September 2026 after clearing \texttt{.cache/}: pass 1 had 0/15 hits and took 48.3\,s; pass 2 had \textbf{15/15 (100\,\%)}, every source took 0.00--0.02\,s, and the pass took 47.5\,s. A perfect hit rate saved under a second because each of the five questions still pays its 6--10\,s synthesis call, which is the strongest argument for the answer cache proposed in \S\ref{sec:limits}. Values that can go stale are exactly the ones we cache: a Wikipedia summary edited after caching, or a new arXiv preprint appearing, will be missed for up to 24 hours. Because the LLM prompt is \emph{not} part of the key and answers are not cached, a prompt change in \texttt{ai/} never serves a stale answer. In production we would add the fetcher version and \texttt{MAX\_SOURCES\_PER\_QUERY} to the key so that changing either parameter invalidates naturally, and add a periodic sweep of expired files.
...
\begin{itemize}[leftmargin=1.4em,itemsep=3pt]
  ...
  \item \textbf{Synthesis is not cached.} Every repeated question pays the LLM call: a second demo pass with 15/15 source-cache hits was only 0.8\,s faster (48.3\,s $\to$ 47.5\,s). Under concurrency the five synthesis calls overlapped fully in our benchmark, so the slowest one sets the wall clock; its latency variance, not the semaphore, keeps the speedup at $4.0\times$ rather than $5\times$. Answer-level caching keyed by \texttt{(question, sorted source URLs, model id)} would make repeated demos near-instant.
  \item \textbf{Retrieval quality is heuristic, and real HTTP is barely tested.} Wikipedia's prefix title search is fed keyword phrases built from an English stopword list tuned on the five sample questions: it can land on a broad article (\texttt{"language models long"} matches only \emph{Language model}), costs up to 8 extra title searches, and a question with no keywords left falls back to its raw text. arXiv ranking, which lives in the provided fetcher, is weak too (the CRISPR question returned particle-physics papers). We never ran more than 5 concurrent questions, never observed a real 429, and have no \texttt{respx} tests of the provided fetchers' network path. With another week we would add those tests, a \texttt{--json} output mode for the CLI, an OpenTelemetry span per external call, and enough repeated benchmark runs to put an error bar on the speedup.
\end{itemize}
...
\texttt{tests/} (67 own tests) & Claude Code & Suggested the test matrix; we reviewed every assertion and kept the concurrency-window design. Provided \texttt{conftest.py} fixtures and \texttt{test\_ai\_smoke.py} are the course's and untouched; our fixtures were appended below them. \\
Wikipedia query fallback in \texttt{services/} \texttt{search\_terms.py} & Claude Code & Diagnosed from the live logs (0 Wikipedia sources for every question); candidate terms were checked against the live endpoint first, then tests and code were written test-first. \\
...
\textbf{Exact commands to reproduce the \S\ref{sec:concurrency} benchmark.} Anyone with the repo and a valid \texttt{.env} should be able to run the lines below and get numbers of the same order as the ones in the table; LLM latency varies from call to call, and our own two sequential timings were 46.8\,s and 71.6\,s.
...
$ pytest --cov=researcher --cov-report=term-missing    # 83 passed, 95% coverage, offline
...
Run the benchmark on a network whose outbound TLS is not intercepted: behind the corporate proxy described in \S\ref{sec:robustness} every fetch failed with a certificate-revocation error and the run ended in \texttt{NoSourcesAvailableError}. Our two runs on 12 September 2026 gave $4.0\times$ (all sources healthy) and $6.1\times$ (one arXiv timeout in the sequential phase).
