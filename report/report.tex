...
\begin{tikzpicture}[
  ...
  box/.style={draw,rounded corners=2pt,minimum width=3.4cm,minimum height=0.85cm,align=center,font=\small},
  ...
  \node[entry,right=of cli] (bench) {\texttt{scripts/bench.py}\\sequential vs \texttt{gather}};
  \node[boot,right=of bench] (boot) {\texttt{bootstrap.py}\\composition root};
  \node[boot,right=of boot,minimum width=2.6cm] (cfg) {\texttt{config.py}\\\texttt{Settings} from \texttt{.env}};

  % --- SE layer --------------------------------------------------------
  \node[core,below=of cli] (fac) {\texttt{core/researcher.py}\\Researcher facade\\{\scriptsize validate $\cdot$ NoSourcesAvailableError $\cdot$ degradation note}};
  \node[core,right=of fac] (orch) {\texttt{concurrency/orchestrator.py}\\gather $\cdot$ timeout $\cdot$ Semaphore(5)\\owns the shared \texttt{AsyncClient}};
  ...
  \node[store,below=of cache] (cs) {\texttt{storage/cache\_store.py}\\\texttt{CacheBackend} ABC\\FS JSON / in-memory};

  % --- provided package and the outside world --------------------------
  ...
  \node[ext,right=of ai] (net) {Wikipedia $\cdot$ arXiv\\Tavily $\cdot$ LLM provider};

  % --- construction (dotted) -------------------------------------------
  \draw[wire] (cli.north) -- ++(0,3mm) -| node[lbl,above,pos=0.25] {\texttt{build\_researcher()}} (boot.north);
  \draw[wire] (bench.north) -- ++(0,3mm);
  \draw[arrow] (boot) -- node[lbl,above] {reads} (cfg);

  % --- runtime calls (solid) -------------------------------------------
  \draw[arrow] (cli) -- node[lbl,left] {\texttt{ask()}} (fac);
  \draw[arrow] (bench.south) -- ++(0,-2.5mm) -| node[lbl,above,pos=0.25] {\texttt{ask()}} (fac.north east);
  ...
  \draw[arrow] (orch) -- node[lbl,above] {\texttt{get()} before semaphore\\\texttt{set()} after fetch} (cache);
  ...
  \begin{scope}[on background layer]
    \node[fit=(fac)(orch)(cache)(ais)(cs),draw=accent!40,dashed,rounded corners=4pt,inner sep=6pt,label={[lbl,anchor=south east]north east:\texttt{researcher/} -- our SE layer}] (se) {};
  \end{scope}
  \draw[wire] (boot.south) -- node[lbl,right] {wires all five objects} (boot.south |- se.north);
\end{tikzpicture}
\end{center}
